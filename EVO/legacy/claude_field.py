...
through the intersection, and applies field-based spatial bias correction.
...
- fields.json: Sensor field polygons for weighted spatial blending

KEY FEATURES:
- Field-Based Spatial Blending: Weighted mosaic approach (no single global reference)
- Path Template Learning: Accumulates knowledge of common trajectories
- Proper Gate Crossing: Uses line segment intersection (not proximity buffering)
- Persistent Learning: Models improve over multiple runs
- Time-Binned Visualization: Splits large datasets into manageable chunks
"""
...
# --- INPUT FILES ---
XML_FILE = "us95&sh8_iprj.txt"       # EVO project configuration file
DATA_FILE = "EVO_raw_data_1770174722.txt"  # Raw radar detection data
GATES_FILE = "gates.json"             # Gate definitions (drawn positions)
FIELDS_FILE = "fields.json"           # Sensor field polygons (trusted regions)
...
PATH_TEMPLATE_MODE = 'augment'        # How to handle existing path templates
SPATIAL_MODEL_MODE = 'augment'        # How to handle existing spatial models

# --- VISUALIZATION ---
TIME_BIN_MINUTES = 5                  # Split output into N-minute chunks
ANIMATION_SPEED_MS = 100              # Frame duration in milliseconds
...
FIELD_BLEND_EPSILON = 1.0             # Smoothing for distance weighting (meters)
...
# --- FIELD-BASED SPATIAL BLENDING ---

def load_or_generate_fields(filepath, df, map_config):
    """
    Load sensor field polygons from JSON, or generate defaults if missing.
    
    Fields define trusted regions for each sensor.
    Returns: list of dicts with 'sensor_id', 'priority', 'polygon' (shapely Polygon)
    """
    if os.path.exists(filepath):
        try:
            ...
            print(f"\n--- Loaded Sensor Fields from {filepath} ---")
            fields = []
            for field_def in data:
                poly = Polygon(field_def['points'])
                fields.append({
                    'sensor_id': field_def['sensor_id'],
                    'priority': field_def.get('priority', 1),
                    'polygon': poly,
                    'name': field_def.get('name', f"S{field_def['sensor_id']}_Field")
                })
                print(f"   {fields[-1]['name']}: {len(field_def['points'])} vertices")
            
            return fields
        except Exception as e:
            print(f"\n⚠️ Warning: Could not load fields from {filepath}: {e}")
            print("   Generating default fields...")
    else:
        print(f"\n--- Generating Default Sensor Fields ---")
    
    # Generate default quadrant-based fields from data extent
    ...
    # Simple quadrant division based on sensor count
    if len(sensors) == 2:
        ...
        fields = [
            {
                'sensor_id': sensors[0],
                'priority': 1,
                'polygon': Polygon([(x_min, y_min), (x_mid, y_min), (x_mid, y_max), (x_min, y_max)]),
                'name': f'S{sensors[0]}_Field_Left'
            },
            {
                'sensor_id': sensors[1],
                'priority': 1,
                'polygon': Polygon([(x_mid, y_min), (x_max, y_min), (x_max, y_max), (x_mid, y_max)]),
                'name': f'S{sensors[1]}_Field_Right'
            }
        ]
    elif len(sensors) >= 4:
        # Quadrant split
        fields = [
            {'sensor_id': sensors[0], 'priority': 1, 
             'polygon': Polygon([(x_min, y_min), (x_mid, y_min), (x_mid, y_mid), (x_min, y_mid)]),
             'name': f'S{sensors[0]}_Field_SW'},
            {'sensor_id': sensors[1], 'priority': 1,
             'polygon': Polygon([(x_mid, y_min), (x_max, y_min), (x_max, y_mid), (x_mid, y_mid)]),
             'name': f'S{sensors[1]}_Field_SE'},
            {'sensor_id': sensors[2], 'priority': 1,
             'polygon': Polygon([(x_min, y_mid), (x_mid, y_mid), (x_mid, y_max), (x_min, y_max)]),
             'name': f'S{sensors[2]}_Field_NW'},
            {'sensor_id': sensors[3], 'priority': 1,
             'polygon': Polygon([(x_mid, y_mid), (x_max, y_mid), (x_max, y_max), (x_mid, y_max)]),
             'name': f'S{sensors[3]}_Field_NE'}
        ]
    else:
        # Fall back: each sensor gets full extent
        for sid in sensors:
            fields.append({
                'sensor_id': sid,
                'priority': 1,
                'polygon': Polygon([(x_min, y_min), (x_max, y_min), (x_max, y_max), (x_min, y_max)]),
                'name': f'S{sid}_Field_Full'
            })
    
    print(f"   Generated {len(fields)} default fields")
    
    # Save generated fields for future use
    save_data = []
    for field in fields:
        save_data.append({
            ...
            'name': field['name'],
            'points': list(field['polygon'].exterior.coords[:-1])  # Remove duplicate last point
        })
...
def get_sensor_weights(x, y, fields, epsilon=FIELD_BLEND_EPSILON):
    """
    Calculate blending weights for a point based on distance to field boundaries.
    
    Uses Inverse Distance Weighting (IDW):
    - Inside one field only: weight = 1.0 for that sensor
    - Inside multiple fields: weight ∝ 1/(distance_to_boundary + ε)
    - Outside all fields: use closest field
    
    Returns: dict {sensor_id: weight} (normalized so sum = 1.0)
    """
    point = Point(x, y)
    weights = {}
    
    # Check which fields contain the point
    containing_fields = [f for f in fields if f['polygon'].contains(point)]
    
    if len(containing_fields) == 1:
        # Inside exactly one field - full weight
        return {containing_fields[0]['sensor_id']: 1.0}
    
    elif len(containing_fields) > 1:
        # Overlap zone - use distance to boundaries
        for field in containing_fields:
            # Distance to boundary (0 if inside, >0 if outside)
            dist_to_boundary = point.distance(field['polygon'].boundary)
            # IDW: closer to center = higher weight
            raw_weight = 1.0 / (dist_to_boundary + epsilon)
            weights[field['sensor_id']] = raw_weight * field['priority']
        ...
        for field in fields:
            dist = field['polygon'].distance(point)
            if dist < min_dist:
                min_dist = dist
                closest_field = field
        
        if closest_field:
            return {closest_field['sensor_id']: 1.0}
        else:
            # Fallback: equal weight to all sensors
            for field in fields:
                weights[field['sensor_id']] = 1.0
...
def apply_field_blended_correction(df, models, fields):
    """
    Apply field-based weighted spatial correction.
    
    For each point:
    1. Calculate weights for all sensors based on field membership
    2. Get corrected position from each sensor (via RBF to S0 reference)
    3. Blend positions using weights: P_final = Σ(P_sensor_i × W_i)
    4. Add columns: BlendedX, BlendedY, DominantSensor
    
    Note: Keeps S0 as mathematical reference for RBF consistency.
    """
    print("\n--- Applying Field-Blended Spatial Correction ---")
    
    if not models and not fields:
        # No correction needed
        df['BlendedX'] = df['X']
        df['BlendedY'] = df['Y']
        df['DominantSensor'] = df['Sensor']
        df['CorrX'] = df['X']  # Keep for backwards compatibility
        df['CorrY'] = df['Y']
        return df
    
    blended_x_list = []
    blended_y_list = []
    dominant_sensor_list = []
    
    for _, row in df.iterrows():
        x, y = row['X'], row['Y']
        original_sensor = row['Sensor']
        
        # Get weights for this location
        weights = get_sensor_weights(x, y, fields)
        
        # Calculate position as seen by each sensor (in S0 reference frame)
        sensor_positions = {}
        
        for sensor_id, weight in weights.items():
            if sensor_id == 0:
                # S0 is reference - use raw position
                sensor_positions[sensor_id] = (x if original_sensor == 0 else None, 
                                              y if original_sensor == 0 else None)
            elif sensor_id in models and original_sensor == sensor_id:
                # Apply RBF correction to align this sensor to S0
                model_data = models[sensor_id]
                rbf_dx = model_data['rbf_dx']
                rbf_dy = model_data['rbf_dy']
                dx = rbf_dx(x, y)
                dy = rbf_dy(x, y)
                sensor_positions[sensor_id] = (x + dx, y + dy)
            elif original_sensor == sensor_id:
                # No model for this sensor, use raw
                sensor_positions[sensor_id] = (x, y)
            else:
                # This sensor didn't detect this point
                sensor_positions[sensor_id] = (None, None)
        
        # Blend positions (only from sensors that detected this point)
        valid_positions = [(sid, pos, weights[sid]) for sid, pos in sensor_positions.items() 
                          if pos[0] is not None and sid in weights]
        
        if not valid_positions:
            # Fallback: use original position
            blended_x_list.append(x)
            blended_y_list.append(y)
            dominant_sensor_list.append(original_sensor)
        elif len(valid_positions) == 1:
            # Only one sensor - use its position
            blended_x_list.append(valid_positions[0][1][0])
            blended_y_list.append(valid_positions[0][1][1])
            dominant_sensor_list.append(valid_positions[0][0])
        else:
            # Weighted blend
            total_weight = sum(w for _, _, w in valid_positions)
            blended_x = sum(pos[0] * w for _, pos, w in valid_positions) / total_weight
            blended_y = sum(pos[1] * w for _, pos, w in valid_positions) / total_weight
            
            # Dominant sensor = highest weight
            dominant_sensor = max(valid_positions, key=lambda v: v[2])[0]
            
            blended_x_list.append(blended_x)
            blended_y_list.append(blended_y)
            dominant_sensor_list.append(dominant_sensor)
    
    df['BlendedX'] = blended_x_list
    df['BlendedY'] = blended_y_list
    df['DominantSensor'] = dominant_sensor_list
    
    # Keep CorrX/CorrY for backwards compatibility with existing code
    df['CorrX'] = df['BlendedX']
    df['CorrY'] = df['BlendedY']
    
    print(f"   Processed {len(df)} points with field-based blending")
    
    return df


# --- END FIELD-BASED SPATIAL BLENDING ---
...
def check_gate_crossings(track_df, gates):
    """
    Check which gates a track crosses using proper line segment intersection.
    
    For each consecutive pair of points (i, i+1), check if the line segment
    between them intersects any gate line using shapely.
    ...
    # Check each line segment in the track
    for i in range(len(track_df) - 1):
        x1, y1 = track_df.iloc[i]['CorrX'], track_df.iloc[i]['CorrY']
        x2, y2 = track_df.iloc[i+1]['CorrX'], track_df.iloc[i+1]['CorrY']
        
        # Create line segment from point i to point i+1
        track_segment = LineString([(x1, y1), (x2, y2)])
        
        for gate_idx, gate in enumerate(gates):
            gate_line = gate['line']
            
            # Check if track segment intersects gate line
            if track_segment.intersects(gate_line) and gate_idx not in gates_hit:
                crossings.append({
                    'sensor': gate['sensor'],
...
if __name__ == "__main__":
    ...
    # 2. Learn/load spatial bias correction (keeps S0 as mathematical reference)
    models, calibration_info = train_spatial_bias_model(df, SPATIAL_MODEL_MODE, saved_spatial)
    
    # 3. Load or generate sensor fields
    fields = load_or_generate_fields(FIELDS_FILE, df, cfg)
    
    # 4. Apply field-blended spatial correction (weighted mosaic approach)
    df_corr = apply_field_blended_correction(df, models, fields)
    
    # 5. Path-based stitching using learned templates (AFTER spatial correction)
    ...
    # 6. Classify tracks by gate crossings
    tracks = classify_tracks_by_gates(df_corr, gates)
    
    # 7. Build/update path templates from high-confidence tracks
    ...
    # 8. Save spatial models if we trained new ones
    ...
    plot_final(fused, internal, discarded, cfg, img, gates, fields, template_data['path_templates'],
